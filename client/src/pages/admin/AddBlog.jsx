      ...
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
         ...
         <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          <div ref={editorRef}></div>
              ...
              <button disabled= {loading} className='absolute bottom-1 right-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer' type='button' onClick={generateContent}>Generate with AI</button>
         </div>
        ...
        <button disabled = {isAdding} className='mt-10 w-40 h-10 bg-primary text-white rounded cursor-pointer text-sm' type='submit'>{isAdding ? 'Adding' : 'Add Blog'}</button>
      </div>
    </form>
  )
}
